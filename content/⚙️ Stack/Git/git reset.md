...
- --soft: change the [[HEAD]] reference to a previous specific commit, reset local tree only (return to status before [[git commit]] but after git add)
- (no flag/ --mixed): default, reset local tree and staging (before git add and [[git commit]])
- --hard: forces the [[HEAD]] to get back to that commit, reset local tree, staging and drive (before git add and [[git commit]], and delete all the new changes in the working directory)
